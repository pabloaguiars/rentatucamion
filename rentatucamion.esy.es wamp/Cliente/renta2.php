...
<html>
	...
	<body>
		<?php
			...
			?>
			<center> <h1> Alta renta </h1> </center>
			<?php
					...
					if($hoy['year'] == $_REQUEST['anio'])
					{
						if($hoy['mon'] == $_REQUEST['mes'])
						{
							if($_REQUEST['dia'] >= $hoy['mday'])
							{
								...
								$registro = mysqli_query($con, "select * from clientes where noCliente = '$_SESSION[IDCliente]'") or die("Error3".mysqli_error($con));
								if($reg = mysqli_fetch_array($registro))
								{
									...
									mysqli_query($con,"insert into rentas(coCamion,noCliente,dia,mes,anio,hora,direccion) values($camionAsignado,$_SESSION[IDCliente],'$_REQUEST[dia]','$_REQUEST[mes]','$_REQUEST[anio]','$_REQUEST[hora]','$direccion')") or die("Error1".mysqli_error($con));
